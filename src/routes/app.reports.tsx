...
function Reports() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from("certificates").select("id, status, created_at, reviewed_at, students(full_name, roll_number, email, departments(name)), domains(name), cohorts(name), ai_verifications(confidence_score)").limit(1000).order("created_at", { ascending: false });
  ...
  function exportCsv() {
    ...
    const csv = ["Roll,Name,Email,Department,Domain,Company,Cohort,Status,AI %,Submitted,Reviewed"].concat(
      rows.map((r: any) => {
        ...
        return [r.students?.roll_number, r.students?.full_name, r.students?.email, r.students?.departments?.name, r.domains?.name, r.cohorts?.name, r.status, aiConf != null ? Math.round(aiConf) : "", r.created_at, r.reviewed_at ?? ""].map((v) => `"${(v ?? "").toString().replace(/"/g, '""')}"`).join(",");
      })
    ).join("\n");
      ...
      <GlassCard className="p-0">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-background/40 text-left text-xs uppercase text-muted-foreground"><tr><th className="p-3">Student</th><th className="p-3">Dept</th><th className="p-3">Domain</th><th className="p-3">AI</th><th className="p-3">Status</th></tr></thead>
